...
export default function TaskBudget({ taskId, task, onUpdateTask }) {  const [budgetOverview, setBudgetOverview] = useState(null);
  ...
  useEffect(() => {
    ...
    if (task?.budget) {
      setBudgetForm({
        totalBudget: task.budget.totalBudget || 0,
  ...
  const fetchBudgetOverview = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/tasks/${taskId}/budget`);
      setBudgetOverview(response.data);
    } catch (error) {
      console.error('Error fetching task budget overview:', error);
    } finally {
      setLoading(false);
    }
  };
  const handleUpdateBudget = async (e) => {
    ...
    try {
      console.log('Updating task budget:', {
        taskId,
        totalBudget: parseFloat(budgetForm.totalBudget),
        currency: budgetForm.currency
      });
      
      const response = await api.put(`/tasks/${taskId}/budget`, {
        totalBudget: parseFloat(budgetForm.totalBudget),
        currency: budgetForm.currency
      });
      ...
      setError(error.response?.data?.message || 'Failed to update budget. Please try again.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h3 className="text-lg font-medium text-gray-900">Task Budget</h3>
          </div>
          <button
        ...
        {budgetOverview?.budget.totalBudget > 0 ? (
                ...
                <div className="flex">
                  ...
                  <div>
                    <h4 className="text-sm font-medium">
                      {budgetStatus === 'exceeded' ? 'Task Budget Exceeded' : 'Task Budget Warning'}
                    </h4>
                    <p className="text-sm mt-1">
                      {budgetStatus === 'exceeded' 
                        ? 'This task has exceeded its allocated budget.'
                        : 'This task is approaching its budget limit.'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Set a budget to track this task's expenses and monitor spending.
            </p>
            <button
